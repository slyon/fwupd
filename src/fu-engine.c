...
fu_engine_ensure_device_battery_inhibit(FuEngine *self, FuDevice *device)
{
	...
	if (fu_device_has_flag(device, FWUPD_DEVICE_FLAG_REQUIRE_AC) &&
		...
		fu_device_inhibit_full(device,
				       FWUPD_DEVICE_INHIBIT_KIND_REQUIRE_AC_POWER,
				       NULL,
				       "Cannot install update when not on AC power");
	} else {
		fu_device_uninhibit_by_kind(device, FWUPD_DEVICE_INHIBIT_KIND_REQUIRE_AC_POWER);
	}
	...
	if (fu_context_get_battery_level(self->ctx) != FU_BATTERY_VALUE_INVALID &&
		...
		fu_device_uninhibit_by_kind(device, FWUPD_DEVICE_INHIBIT_KIND_SYSTEM_POWER_TOO_LOW);
	}
}
...
fu_engine_ensure_device_lid_inhibit(FuEngine *self, FuDevice *device)
{
	...
	fu_device_uninhibit_by_kind(device, FWUPD_DEVICE_INHIBIT_KIND_LID_IS_CLOSED);
}
